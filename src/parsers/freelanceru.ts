...
export class FreelanceruParser implements Parser {
  ...
  async fetchOrders(): Promise<Order[]> {
    if (!config.freelanceru.enabled) {
      console.log(
        "⏭️  [freelanceru] Парсер отключён (FREELANCERU_ENABLED != true)",
      );
      return [];
    }
      ...
      const raw = await page.$$eval(cardSelector, (cards) => {
        return cards
          .map((card) => {
            ...
            const offersCount = 0;
            const timeEl = card.querySelector("time.timeago");
  ...
  private async findWorkingSelector(
    page: Awaited<
      ReturnType<
        Awaited<ReturnType<typeof createBrowser>>["context"]["newPage"]
      >
    >,
    selectorList: string,
  ): Promise<string | null> {
    ...
    for (const sel of candidates) {
      try {
        ...
        if (count > 0) {
          console.log(
            `✅ [freelanceru] Селектор работает: "${sel}" (${count} элементов)`,
          );
          return sel;
        }
      } catch {
