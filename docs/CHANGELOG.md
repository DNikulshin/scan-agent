...
## 2026-05-13 — Ревизия cron-workflow `ScanAgent`

Аналитика по 400 последним прогонам (GitHub API) + `run_metrics`:
- Success rate ~98% (393/400). p50 длительности 209s, p90 298s, max 9598s (1 аномальный workflow_dispatch завис на 160 мин). Прогонов > 30 мин — 3.
- ~~`run_metrics` пишется только в ~14 из ~80 ожидаемых строк за 5 дней~~ — диагностика: первая успешная запись 2026-05-12 22:55 UTC, через 42 мин после коммита `fbffb84` (PrismaClient переведён на `datasourceUrl: process.env.DATABASE_URL`). До фикса агент в GHA падал на любом SQL, не доходя до `metrics.flush()`. Проблема **уже решена**, начиная с 12 мая 22:13 UTC.
- Расхождение комментариев YAML с фактом: «07:00–10:00 МСК» / «17:00–20:00 МСК» — на самом деле до 09:30 и 19:30. Внепик в YAML был задан в UTC, но в DEPLOY.md описывался как МСК.

**Правки в `.github/workflows/scan-agent.yml`:**
- `concurrency: scan-agent-run` (cancel-in-progress=false) — параллельные прогоны теперь стоят в очереди, не накладываются.
- `timeout-minutes: 25` — потолок < 30-минутного cron-интервала, защита от зависших прогонов (дефолтные 360 мин больше не сжигаются).
- Кеш браузеров Playwright (`actions/cache@v4` на `~/.cache/ms-playwright`) — экономит 30–60с на каждом из 20 прогонов/сутки.
- Удалены закомментированные шаги `agent.db` (легаси SQLite).
- Комментарии cron исправлены: явно указано «UTC, МСК = UTC+3» и фактические окна 07:00–09:30 / 17:00–19:30 МСК + список внепиковых МСК-часов.

**Правки в `docs/DEPLOY.md`:** добавлена точная таблица расписания и пояснение про `concurrency`/`timeout-minutes`.
